 ...
 * Authentication and profile reads are enforced through Firebase SDK rules.
 */
(function(){
  'use strict';
  ...
  var script=document.currentScript, rootUrl='';
  try{ if(script && script.src) rootUrl=new URL('.',script.src).href; }catch(e){}
  if(!rootUrl) rootUrl=new URL('./',location.href).href;

  var path=location.pathname||'', file=(path.split('/').pop()||'index.html').toLowerCase();
  if(file==='login.html') return;

  var RETURN_KEY='smc_auth_return', PROFILE_KEY='smc_account';
  var cfg=null, auth=null, db=null;
  ...
  function hidePage(){
    var s=document.createElement('style');s.id='smc-auth-style';
  ...
  function loadScript(src){return new Promise(function(resolve,reject){var s=document.createElement('script');s.src=src;s.onload=resolve;s.onerror=function(){reject(new Error(src));};document.head.appendChild(s);});}
  function loadFirebase(){
    var p=Promise.resolve();
    if(!window.SMC_FIREBASE_CONFIG) p=p.then(function(){return loadScript(new URL('firebase-config.js',rootUrl).href);});
    p=p.then(function(){
      cfg=window.SMC_FIREBASE_CONFIG||{};
      if(!window.firebase||!firebase.initializeApp) return loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-app-compat.js');
    }).then(function(){
      if(!window.firebase||!firebase.auth) return loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-auth-compat.js');
    }).then(function(){
      if(!window.firebase||!firebase.database) return loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-database-compat.js');
    }).then(function(){
      cfg=window.SMC_FIREBASE_CONFIG||{};
      if(!cfg.apiKey||!cfg.authDomain||!cfg.appId||!cfg.databaseURL) throw new Error('Firebase config missing');
  ...
  function audit(user,a){
    try{db.ref('auth_audit').push({uid:user.uid,event:'authenticated',page:location.pathname,t:new Date().toISOString(),email:user.email||'',plan:a.plan||'free'}).catch(function(){});}catch(e){}
  }
  function chip(user,a){
    ...
    c.querySelector('#smcAuthLogout').onclick=function(){auth.signOut().then(function(){try{localStorage.removeItem(PROFILE_KEY);}catch(e){}location.replace(loginUrl());});};
  }
  function start(){
    overlay();
    loadFirebase().then(function(){
      auth.onAuthStateChanged(function(user){
        ...
        profile(user).then(function(p){
          ...
          setTimeout(function(){var o=document.getElementById('smcAuthOverlay');if(o)o.remove();showPage();chip(user,a);},120);
        }).catch(function(){fail('Your account session is valid, but the profile could not be verified. Please sign in again.');});
      });
