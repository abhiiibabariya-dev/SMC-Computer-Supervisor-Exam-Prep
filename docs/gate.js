 ...
 * The old name/mobile unlock modal is intentionally removed.
 * Subscription metadata: free / premium49 / premium99.
 */
(function(){
  'use strict';
  ...
  function loadFirebase(){
    ...
    if(!window.SMC_FIREBASE_CONFIG)p=p.then(function(){return loadScript('firebase-config.js');});
    p=p.then(function(){cfg=window.SMC_FIREBASE_CONFIG||{};if(!window.firebase||!firebase.initializeApp)return loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-app-compat.js');})
     .then(function(){if(!window.firebase||!firebase.auth)return loadScript('https://www.gstatic.com/firebasejs/10.12.5/firebase-auth-compat.js');})
     .then(function(){cfg=window.SMC_FIREBASE_CONFIG||{};if(!cfg.apiKey||!cfg.authDomain||!cfg.appId)throw new Error('Firebase config missing');if(!firebase.apps.length)firebase.initializeApp(cfg);auth=firebase.auth();});
    return p;
  }
  function overlay(){
  ...
  function chip(user,a){
    ...
    document.body.appendChild(c);c.querySelector('#smcAuthLogout').onclick=function(){auth.signOut().then(function(){try{localStorage.removeItem(PROFILE_KEY);}catch(e){}location.replace(loginUrl());});};
  }
  function start(){
    overlay();
    loadFirebase().then(function(){auth.onAuthStateChanged(function(user){
      if(!user){showPage();location.replace(loginUrl());return;}
      profile(user).then(function(p){var a=account(user,p);audit(user,a);var m=document.getElementById('smcAuthMsg'),s=document.getElementById('smcAuthState');if(m)m.textContent='Signed in as '+(a.name||user.email||'your account')+'.';if(s)s.textContent='ACCESS GRANTED';setTimeout(function(){var o=document.getElementById('smcAuthOverlay');if(o)o.remove();showPage();chip(user,a);},120);});
