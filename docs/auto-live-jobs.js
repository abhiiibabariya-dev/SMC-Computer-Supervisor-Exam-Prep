...
(function(){
  ...
  function eventSummary(status){
    ...
    return events.map(e=>({
      text:`${e.title||'SMC exam update'}${e.date?' • '+fmtDate(e.date):''}`,
      status:labels[e.status]||String(e.status||'UPDATE').toUpperCase(),
      cadres:Array.isArray(e.cadres)?e.cadres:[],
      url:e.source||'https://www.suratmunicipal.gov.in/Information/RecruitmentNews'
    }));
  }
  ...
  function replaceStaleRenderedText(status){
    ...
    const postponed=events.find(e=>e.status==='POSTPONED');
  ...
  function styles(){
    ...
    s.textContent=`#gj-live-updates{max-width:1100px;margin:92px auto 24px;padding:0 18px;position:relative;z-index:20;font-family:inherit}#gj-live-updates .gj-box{background:linear-gradient(135deg,rgba(10,20,16,.97),rgba(10,14,22,.97));border:1px solid rgba(34,197,94,.22);border-radius:18px;padding:18px;box-shadow:0 12px 40px rgba(0,0,0,.25)}#gj-live-updates .gj-head{display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:12px}#gj-live-updates .gj-title{font-size:1rem;font-weight:900;color:#f4f4f5}#gj-live-updates .gj-live{font-size:.68rem;font-weight:800;color:#86efac;white-space:nowrap}#gj-live-updates .gj-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:9px}#gj-live-updates .gj-item{display:block;text-decoration:none;color:inherit;background:rgba(255,255,255,.035);border:1px solid rgba(255,255,255,.07);border-radius:12px;padding:12px}.gj-status{font-size:.62rem;font-weight:900;color:#fde68a;margin-top:6px}.gj-cadres{font-size:.64rem;color:#a1a1aa;margin-top:4px}#gj-live-updates .gj-org{font-size:.64rem;font-weight:800;letter-spacing:.7px;text-transform:uppercase;color:#a7f3d0;margin-bottom:4px}#gj-live-updates .gj-name{font-size:.8rem;line-height:1.4;font-weight:800;color:#f4f4f5}#gj-live-updates .gj-tag{display:inline-block;margin-top:7px;padding:3px 8px;border-radius:999px;font-size:.58rem;font-weight:800;background:rgba(34,197,94,.1);color:#86efac}#gj-live-updates .gj-footer{margin-top:11px;text-align:right;font-size:.65rem;color:#71717a}@media(max-width:600px){#gj-live-updates{margin-top:82px;padding:0 10px}#gj-live-updates .gj-box{padding:13px}#gj-live-updates .gj-grid{grid-template-columns:1fr}}`;
  ...
  function renderJobs(data,status){
    ...
    styles();
    const items=data.items.filter(x=>x&&x.url&&x.title).sort((a,b)=>rank(a)-rank(b)).slice(0,60);
    const events=eventSummary(status);
