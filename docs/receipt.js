...
window.SMCReceipt={
 ...
 email:async function(data){
  ...
  var p={to_email:data.email,name:data.name||'',order:data.orderId||'',plan:data.planLabel||data.plan||'',amount:money(data.amount),txn:data.txnId||'',date:date(data.approvedAt||data.createdAt),uid:data.uid||'',expiry:data.expiresAt?date(data.expiresAt):'Lifetime',receipt_id:data.receiptId||data.orderId||'',receipt_pdf:pdfData};
